...
public class ChatGuruAdapter extends BaseAdapter {

    Funct funct;
    private List<ChatGuru> list;
    private Context context;

    public ChatGuruAdapter(Context context, List<ChatGuru> list){
        this.list = list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ChatGuru chat = list.get(position);
        ...
        if(chat.getFoto_siswa() != null && chat.getFoto_siswa().length() > 0){
            Picasso.get().load("http://192.168.43.206/bimbel/public/fotosiswa/" + chat.getFoto_siswa()).placeholder(R.drawable.placeholder).into(imgChat);
        } else{
